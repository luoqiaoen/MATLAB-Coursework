% incoh_image Incoherent Imaging Example
 ...
 OTFd=fftshift(abs(OTF/OTF(1,1)));
 figure(1)
 disp('optical transfer function')
 imagesc(abs(OTFd))
 pause
 figure(2) %check OTF
 disp('point spread function')
...
pause
% linear system image generation
 ...
 disp('blurred image due to linear filtering')
 imagesc(u,v,nthroot(Ii,2));
 colormap('gray'); xlabel('u (m)'); ylabel('v (m)');
 axis square;
 axis xy;
pause
disp('horizontal slice from blurred image due to linear filtering')
