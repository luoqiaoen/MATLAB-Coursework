...
if numel(n) > 1
   ...
   C=c(1+n,1+N); % root
   c=c(1+n,1:N); % Read and store the vector of roots of the pth-order Bessel function
   R,C,pause
   r=R/C*c(:); %Calculate the radius vector
   k=c(:)/R;%the radial angular frequency (did not divide by 2pi like the paper did)
   I=abs(besselj(1+n,c)); %denominator of the transmission matrix
   K=2*pi*R/C*I(:);%
   R=I(:)/R;%column vector J 
   I=sqrt(2/C)./I;
   I=I(:)*I.*besselj(n,c(:)/C*c);%;transmission matrix
end
size(I),pause
if isempty(h)
   H=h;
